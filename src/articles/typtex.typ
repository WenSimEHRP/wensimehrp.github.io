...
Both #elink("https://typst.app/")[Typst] and #elink("https://www.latex-project.org/")[Latex]
are powerful typesetting tools. Typst, the newborn, uses a rust-like#footnote[
  I have to be honest with you: I haven't written that much rust code _yet_. Don't take my words
  seriously as if they are from a rust professional.
]
...
This, of course, is not real Latex code. Anyways, backslashes are usually used as
the #elink("https://en.wikipedia.org/wiki/Escape_character")[escape character] in
modern systems. Typst also use the backslash as an escape character, but only for
entering special characters. For code and function calls, Typst uses "\#", the pound.
...
generated by text macros can be parsed into valid #elink("https://en.wikipedia.org/wiki/Abstract_syntax_tree")[ASTs].
...
Once a variable is out of its lifetime, or functioning zone #footnote[
  I read too many rust tutorials in Chinese and I completely forgot
  the correct English name.
]. Even when using macros
we still have to follow Typst's rules. Next, we would need to use a macro processor. Something like
#elink("https://www.gnu.org/software/m4/")[GNU M4] would work very well, the standard C/C++ preprocessor
